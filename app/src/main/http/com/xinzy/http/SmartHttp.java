...
public final class SmartHttp {
     ...
     * @return
     */
    public <T> T enqueue(@NonNull TypeToken<T> typeToken, @Nullable RequestCallback<T> callback) {
    ...
    private <T> T enqueue(final Type type, RequestCallback<T> rawCallback) {
        checkInit();
        ...
        call.enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                error(callback, new SmartHttpException(e));
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (!response.isSuccessful()) {
                    error(callback, new SmartHttpException("http request error and error code is " + response.code()));
                    return;
                }
                ...
                try {
                    ...
                    new Handler(Looper.getMainLooper()).post(() -> callback.onSuccess(val));
                } catch (JsonSyntaxException e) {
                    Utils.e("parse fail json string: " + content, e);
                    error(callback, new SmartHttpException("Gson parse json fail", e));
                } catch (Exception e) {
                    Utils.e("", e);
                    error(callback, new SmartHttpException(e));
                }
    ...
    private <T> void error(RequestCallback<T> callback, SmartHttpException e) {
        new Handler(Looper.getMainLooper()).post(() -> callback.onFailure(e));
    }

    private <T> T checkCache(Type type) {
        if (mCacheModel != FIRST_CACHE_THEN_REQUEST) return null;
        if (type == null) {
            return (T) mCache.get(mCacheKey);
        } else {
    ...
    public static class DefaultCallback<T> implements RequestCallback<T> {
